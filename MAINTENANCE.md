...
- **Export format (v2):** `noteds-backup-<timestamp>.zip` containing `data.json`. `data.json` carries `backupVersion = 2`, customers, and ledger entries; each photo field stores both a relative path (for older archives) **and** an optional `*Base64` payload that inlines the image bytes when the source file exists. Photos are still mirrored into `photos/` during export for backward compatibility, but restore now prefers the Base64 payload.
